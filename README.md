...
1. 选择输入文件夹，可选“子目录”；Windows 下也可以直接把一个文件夹从资源管理器拖入主窗口，程序会写入路径并开始扫描。
2. 在右上角选择内容类型：`番剧` 或 `轻小说`。
3. 设置作品名 / 系列名、命名规则及需要的选项，点击“扫描文件”。
4. 在“未来记录 / RENAME PREVIEW”中检查原名、目标名、识别结果和状态。
5. 可使用顶部筛选栏按原文件名、目标文件名或分组搜索，并按 `全部 / 可执行 / 待确认 / 冲突或错误 / 已跳过` 过滤；“只看问题”和“全选当前”可加快批量检查。
6. 必要时使用“分组”“纠正 / 纠正卷”“跳过/恢复”“清除”进行人工调整。
7. 改动输入路径、命名设置或内容类型后，需要重新扫描，`改写未来` 才会重新启用。
8. 确认预览无冲突后，点击“改写未来”执行。

单击一行会显示完整源路径、目标路径及识别依据；双击可进入对应纠正操作。规则评分只用于提示匹配强弱，不是统计准确率。
...
双击轻小说条目可人工纠正卷号；轻小说模式会切换为“书库源 / 系列 / 起始卷 / 纠正卷”等语义，并隐藏季度、字幕联动和语言标签等番剧专属控件；“顺序编号”可用于按组重新分配卷号。预览表会使用 `VOLUME / 卷信息` 列显示识别出的卷号及相关信息，并在切换轻小说模式时重新分配列宽，避免标题被截断。


### 当前交互要点

- Windows 默认窗口目标约 `1392×864`，并按当前屏幕可用区域限制最大尺寸。
- 支持把单个文件夹从资源管理器直接拖入窗口；拖入单个文件、多个路径或无效路径时不会擅自执行。
- 预览区支持文本搜索、状态筛选、“只看问题”和“全选当前”，便于大量文件时快速定位异常项。
- 番剧与轻小说共用同一套执行 / 撤销 / 恢复底层，但轻小说会切换为“书库源 / 系列 / 卷信息 / 起始卷 / 纠正卷”等专属语义。
- 当前项目版本为 `0.3.2`；本地构建输入更高三段版本号后会记住该版本，后续直接回车沿用，较低版本不会使项目版本倒退。
...
当前 UI 已进入 Preview 19.14 阶段。除 Future Diary 主题状态横幅外，现已包含更大的默认窗口、轻小说专属语义、Windows 原生文件夹拖入、预览搜索与状态筛选，以及复制进度 / 安全取消等交互。主要资源包括：
...
当前布局还包含内容类型选择器、复制实时进度与安全取消、预览搜索 / 状态筛选、“只看问题”“全选当前”、轻小说专属字段，以及空预览居中和预览区域对齐修正。默认窗口目标尺寸已由约 `1160×720` 提升到约 `1392×864`，小屏幕仍会按可用桌面空间自动收缩。
...
- 侧边栏角色图和横幅仍使用项目内现有 PNG 素材；运行时使用原生 Tk 图像资源，不对整个窗口做低分辨率截图式缩放。高分辨率派生源 `assets/yuno_sidebar_hd.png` 保留用于后续重新裁切与素材优化。
